...
出力:
- approve=false は「明確な却下理由」がある場合のみ(織り込み済みが濃厚、
  定型的・軽微な開示、アナリストの根拠が数字と明確に矛盾、など)。
- 判断が拮抗する場合は approve=true とし、action や priority を保守的に
  修正する(例: '今すぐ'→'押し目待ち'、priority を下げる)方を選ぶ。
